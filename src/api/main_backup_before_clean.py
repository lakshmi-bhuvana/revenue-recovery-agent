...
def apply_scenario_recovery_policy(
    ...
    # ------------------------------------------------------------
    # RECOVERY POLICY / ESCALATION BOUNDARY
    # ------------------------------------------------------------
    # Policy is based on the server-owned number of recovery attempts
    # BEFORE this request is executed.
    #
    # HIGH   : first attempt is allowed; second attempt escalates.
    # MEDIUM : first 3 attempts are allowed; 4th attempt escalates.
    # LOW    : first 3 attempts are allowed; 4th attempt escalates.
    #
    # An escalation boundary is a policy rejection: no execution,
    # no counter increment, and no money recovery. The rejection is
    # still returned in the final result and therefore captured by audit.
    attempts = safe_int(transaction.get("recovery_attempts", 0))
    priority = safe_string(score.get("priority", "LOW"), "LOW").upper()

    if scenario not in SUPPORTED_SCENARIOS:
        policy["allowed"] = False
        policy["reason"] = "unsupported_recovery_scenario"
    elif priority == "HIGH" and attempts >= 1:
        policy["allowed"] = False
        policy["reason"] = "high_priority_escalation_required"
    elif priority in {"MEDIUM", "LOW"} and attempts >= 3:
        policy["allowed"] = False
        policy["reason"] = "maximum_recovery_attempts_reached"
    else:
        policy["allowed"] = True
    ...
    # Policy rejection must happen before execution.
    if not policy.get("allowed", False):
        execution.update({
            ...
            "money_recovered": 0.0,
            "execution_detail": (
                "Recovery action blocked because policy requires escalation."
                if policy["reason"] == "high_priority_escalation_required"
                else "Recovery action blocked because the maximum number of recovery attempts has been reached."
                if policy["reason"] == "maximum_recovery_attempts_reached"
        ...
        if policy["reason"] == "high_priority_escalation_required":
            escalation.update({
                ...
                "reason": "high_priority_escalation_required",
                "recommended_team": "recovery_specialist",
            })
        elif policy["reason"] == "maximum_recovery_attempts_reached":
            escalation.update({
                "escalate": True,
        ...
        execution["attempt_increment"] = 1
        execution["message_sent"] = True
        execution["recovered"] = recovered
        execution["money_recovered"] = money_recovered
        ...
        execution["attempt_count"] = attempts + 1
        stopping.setdefault(
