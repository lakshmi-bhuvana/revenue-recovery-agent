...
# Server-owned hard boundary.
#
# Attempt 1 -> recovery_attempts 0 before execution
# Attempt 2 -> recovery_attempts 1 before execution
# Attempt 3 -> recovery_attempts 2 before execution -> BLOCKED
#
# Therefore the maximum number of actual recovery executions is 2.
...
LIVE_EVENTS_FILE = (
    BASE_DIR / "data" / "raw" / "recovery_live_events.json"
)
...
def load_live_events() -> None:
    ...
    try:
        ...
        payload = json.loads(
            LIVE_EVENTS_FILE.read_text(encoding="utf-8")
        )
        ...
        # A corrupt optional live-state file must never prevent the API
        # from starting; the base dataset remains available.
        PROCESSED_RECOVERY_EVENTS = {}
        LIVE_RECOVERY_STATE = {}
...
SCENARIO_DIAGNOSIS_REASONS = {
    ...
    "b2b_receivable": (
        "A B2B receivable is overdue and requires collection "
        "follow-up."
    ),
    "mandate_failure": (
...
class RecoveryEvent(BaseModel):
    ...
    payment_method_success_rate: float = Field(
        default=0.8,
        ge=0,
        le=1,
    )
    ...
    product_interest_score: float = Field(
        default=0.5,
        ge=0,
        le=1,
    )

    checkout_progress: float = Field(
        default=0.5,
        ge=0,
        le=1,
    )

    customer_email_available: int = Field(
        default=1,
        ge=0,
        le=1,
    )

    customer_phone_available: int = Field(
        default=1,
        ge=0,
        le=1,
    )
...
def get_at_risk_data() -> pd.DataFrame:
    ...
    if PROCESSED_RECOVERY_EVENTS:
        ...
        # Recovery events are an event stream, so they are appended
        # to the dataset rather than replacing a historical row with
    ...
    # Re-apply authoritative values from the live agent result.
    if "_agent_result" in df.columns:

        for idx, raw_result in (
            df["_agent_result"].items()
        ):
...
def dashboard_summary(
    ...
    intervention_cases = int(
        (retry_series > 0).sum()
    )

    recovery_amount_per_intervention = (
        actual_recovered
        / intervention_cases
        if intervention_cases > 0
        else 0.0
    )

    scenario_recovery: dict[
        str,
        dict[str, Any],
    ] = {}
    ...
    if "scenario" in at_risk.columns:
        ...
        for scenario_name in sorted(
            ...
            scenario_recovered_cases = int(
                scenario_df[
                    "recovered"
                ].sum()
            )

            scenario_money_recovered = float(
                scenario_df[
                    "money_recovered"
                ].sum()
            )

            scenario_recovery_rate = (
                scenario_recovered_cases
                / scenario_cases
                * 100
                if scenario_cases
                else 0.0
...
def apply_scenario_recovery_policy(
    ...
    """
    Apply API-level scenario policy after the agent runs.

    The maximum-attempt policy is handled before the agent is called
    in /recovery-events. This function therefore handles the normal
    scenario policy and preserves an existing hard policy rejection.
    """
    ...
    # ------------------------------------------------------------
    # HARD POLICY PRESERVATION
    # ------------------------------------------------------------
    #
    # If the agent or an earlier server-side policy has already
    # explicitly denied the action, NEVER convert that denial into
    # allowed=True.
    #
    # The maximum-attempt boundary is normally handled before the
    # agent is called, but this safeguard prevents accidental policy
    # inversion if another policy rejection reaches this function.
    # ------------------------------------------------------------

    existing_policy_allowed = policy.get(
        "allowed"
    )

    existing_policy_reason = safe_string(
        policy.get("reason")
    ).strip()

    if (
        existing_policy_allowed is False
        and existing_policy_reason
        ...
        execution.update({
            ...
            "money_recovered": 0.0,
            "execution_detail": (
                "Recovery action blocked because "
                "the maximum number of recovery "
...
# ============================================================
# SERVER-SIDE POLICY BLOCK RESULT
# ============================================================
...
def build_policy_blocked_result(
    ...
    """
    Build a deterministic audit result when the server-owned
    maximum recovery attempt policy rejects an event.

    The RecoveryAgent is intentionally NOT called for this case.
    """
...
def run_recovery_agent(
    ...
    return apply_scenario_recovery_policy(
        transaction,
        result,
    )
...
async def health():
    ...
    return {
        ...
        "live_events_persisted": (
            LIVE_EVENTS_FILE.exists()
        ),
        "live_events_file": str(
            LIVE_EVENTS_FILE
        ),
...
async def process_recovery_event(
    ...
    # ------------------------------------------------------------
    #
    # The client is NOT trusted to determine the current attempt.
    ...
    # ------------------------------------------------------------
    # SERVER-OWNED MAX-ATTEMPT POLICY
    # ------------------------------------------------------------
    #
    # IMPORTANT:
    #
    # Do this BEFORE calling RecoveryAgent.
    #
    # This prevents the agent's older internal attempt policy from
    # becoming the authoritative policy boundary.
    #
    # Attempt count before execution:
    #
    # 0 -> allowed -> becomes 1
    # 1 -> allowed -> becomes 2
    # 2 -> blocked -> remains 2
    # 3 -> blocked -> remains 3 only if already stored elsewhere
    #
    # In this application, the counter stops at 2.
    # ------------------------------------------------------------
    ...
    if current_attempts >= MAX_RECOVERY_ATTEMPTS:
        ...
        try:
            ...
            # Ensure the server-owned attempt count is present.
            execution[
                "attempt_count"
            ] = current_attempts
            ...
            # ----------------------------------------------------
            #
            # The rejected request is deliberately persisted.
            # This means a policy rejection remains visible after
            # API restart and contributes to the event audit trail.
            # ----------------------------------------------------
    ...
    try:
        ...
        # Defensive boundary:
        # the server never permits the counter to exceed the
        # configured maximum through a normal execution.
        next_attempt_count = (
            current_attempts
            + attempt_increment
        )

        if next_attempt_count > (
            MAX_RECOVERY_ATTEMPTS
        ):
            next_attempt_count = (
                MAX_RECOVERY_ATTEMPTS
