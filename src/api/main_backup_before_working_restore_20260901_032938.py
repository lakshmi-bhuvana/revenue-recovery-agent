...
PROCESSED_RECOVERY_EVENTS: dict[
    str,
    dict[str, Any],
] = {}

LIVE_RECOVERY_STATE: dict[
    str,
    dict[str, Any],
] = {}
...
def persist_live_events() -> None:
    """
    Persist all processed live recovery events to disk.

    The runtime JSON file allows recovery state to survive
    ...
    RECOVERY_EVENTS_FILE.write_text(
        json.dumps(
            payload,
            indent=2,
            ensure_ascii=False,
...
def load_live_events() -> None:
    """
    Load previously persisted live recovery events.

    If the runtime file does not exist or is invalid,
    the application continues with an empty live state.
    """

    if not RECOVERY_EVENTS_FILE.exists():
        return

    try:

        payload = json.loads(
            RECOVERY_EVENTS_FILE.read_text(
                encoding="utf-8"
            )
        )
        ...
        PROCESSED_RECOVERY_EVENTS.update(
            events
        )

        for transaction_id, row in events.items():
...
def calculate_recovery_probability(
    ...
    probability = (
        ...
        * df["customer_success_rate"]
        + 0.20
        * df["product_interest_score"]
        + 0.15
        * df["payment_method_success_rate"]
        + 0.10
        * df["checkout_progress"]
    )
...
def get_at_risk_data() -> pd.DataFrame:
    ...
    Live recovery events are stored in memory and persisted to
    data/runtime/recovery_events.json so their state survives
    FastAPI restarts.

    The cache is invalidated automatically when the CSV changes.
    """
    ...
    if PROCESSED_RECOVERY_EVENTS:
        ...
        # Remove historical copy if transaction was processed live.
        df = df[
            ~df[
                "transaction_id"
            ]
...
async def health():
    ...
    return {
        ...
        "recovery_events_file": str(
            RECOVERY_EVENTS_FILE
        ),
        "recovery_events_file_exists":
            RECOVERY_EVENTS_FILE.exists(),
    }
