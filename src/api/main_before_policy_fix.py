...
# Maximum automated recovery attempts per transaction.
...
SCENARIO_DIAGNOSIS_REASONS = {
    ...
    "overdue_receivable": (
        "An overdue receivable requires collection follow-up."
    ),
...
PAYMENT_FAILURE_DIAGNOSES = {
    ...
    "insufficient_funds": {
        ...
        "reason": "The payment failed because sufficient funds were unavailable.",
    },
    "card_declined": {
        "diagnosis": "card_decline",
        "reason": "The card payment was declined.",
    },
    "payment_timeout": {
        "diagnosis": "payment_timeout",
        "reason": "The payment attempt timed out before completion.",
    },
    "gateway_error": {
        "diagnosis": "gateway_error",
        "reason": "The payment failed because of a payment gateway error.",
    },
}
# ============================================================
# REQUEST MODELS
...
def apply_scenario_recovery_policy(
    ...
    """Apply authoritative scenario policy, execution bounds, and escalation."""
    ...
    escalation = dict(result.get("escalation") or {})

    if not diagnosis.get("diagnosis"):
    ...
    action["strategy"] = defaults["strategy"]
    action["channel"] = defaults["channel"]
    action["scenario"] = scenario

    attempts = safe_int(transaction.get("recovery_attempts", 0))
    priority = safe_string(score.get("priority", "LOW"), "LOW").strip().upper()

    # Policy boundary:
    # HIGH: attempt 1 is the only automated recovery; attempt 2+ escalates.
    # MEDIUM/LOW: attempts 1-3 are automated; attempt 4+ escalates.
    escalation_required = (attempts >= 1) if priority == "HIGH" else (attempts >= 3)
    ...
    if scenario not in SUPPORTED_SCENARIOS:
        ...
        policy["allowed"] = False
        policy["reason"] = "recovery_escalation_required"
    else:
        policy["allowed"] = True
    ...
    if not policy["allowed"]:
        execution.update({
            ...
            "money_recovered": 0.0,
            "execution_detail": (
                "Recovery action blocked because escalation is required."
                if escalation_required
                else "Action blocked by recovery policy."
            ),
            "scenario": scenario,
        ...
        stopping.update({
            "stop": True,
            "reason": "RECOVERY_ESCALATION_REQUIRED" if escalation_required else "POLICY_BLOCKED",
        })
        if escalation_required:
            escalation.update({
                "escalate": True,
        ...
        transaction_amount = safe_float(transaction.get("transaction_amount", 0.0))
        recovery_probability = safe_float(
            transaction.get("_recovery_probability", score.get("recovery_probability", 0.0))
        )
        force_failure = bool(transaction.get("_force_recovery_failure", False))
        recovered = False if force_failure else recovery_probability >= 0.70
        ...
        execution.update({
            ...
            "message_sent": True,
            "recovered": recovered,
            "money_recovered": transaction_amount if recovered else 0.0,
            "scenario": scenario,
        ...
        if recovered:
            execution["execution_detail"] = (
                f"Recovery succeeded through {action['recovery_action']} "
                f"using {action['channel']} for {scenario}."
            )
            stopping.update({"stop": True, "reason": "PAYMENT_SUCCESS"})
            escalation.update({
                "escalate": False,
                "escalation_level": "NONE",
            ...
            execution["execution_detail"] = (
                f"Recovery attempt executed through {action['recovery_action']} "
                f"using {action['channel']}, but payment was not recovered."
            )
            stopping.update({"stop": True, "reason": "RECOVERY_FAILED"})
            escalation.update({
                "escalate": False,
                "escalation_level": "NONE",
...
def run_recovery_agent(
    ...
    agent_transaction = dict(transaction)
    if safe_int(transaction.get("recovery_attempts", 0)) >= MAX_RECOVERY_ATTEMPTS:
        agent_transaction["recovery_attempts"] = 0
        agent_transaction["_force_recovery_failure"] = True
        agent_transaction["force_recovery_failure"] = True

    result = agent.process(agent_transaction)
    if not isinstance(result, dict):
        raise ValueError("RecoveryAgent returned an invalid result.")

    result = apply_scenario_recovery_policy(transaction, result)
    result["audit"] = build_audit(transaction, result)
    return result
