...
async def customer_detail_api(customer_id: str):
    ...
    for transaction_id in matches["transaction_id"].astype(str):
    ...
    total_value = sum(
        safe_float(x["transaction_amount"])
        for x in transactions
    )

    recovered_cases = sum(
        1 for x in transactions if x["recovered"]
    )

    money_recovered = sum(
        safe_float(x["money_recovered"])
        for x in transactions
    )
