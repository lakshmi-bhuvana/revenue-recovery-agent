...
def get_at_risk_data() -> pd.DataFrame:
    ...
    calculated_strategy = assign_strategy(
        at_risk["priority_score"]
    )
    ...
    if "strategy" not in at_risk.columns:
        ...
        strategy.loc[missing] = (
            calculated_strategy.loc[missing]
        )
...
def get_ai_answer(
    ...
    if q in {
        ...
        return (
            ...
            "I analyze failed-payment recovery opportunities and help "
            "you understand where revenue is at risk, which cases should "
            "be prioritized, and why the recovery agent selected a "
            "particular action.\n\n"
            "You can ask:\n"
    ...
    if reasoning_requested and conversation:
        ...
        # If previous question was about prioritization
        if (
            ...
            return (
                ...
                "It combines the amount that could be recovered with "
                "the probability of successfully recovering it.\n\n"

                f"For this case, {format_inr(row['transaction_amount'])} "
                f"is at risk and the estimated recovery probability is "
                f"{row['recovery_probability'] * 100:.2f}%. That produces "
                f"an expected recovery of "
                f"{format_inr(row['expected_recovery_value'])}.\n\n"

                "Because the case also has strong customer intent and "
                "customer payment history, the system considers it a "
        ...
        # If previous question was about failure
        if (
            ...
            return (
                f"Some recovery cases fail because the underlying "
                f"payment problem is difficult to recover from.\n\n"
                ...
                "payment-method success rate, and the recovery channel "
                "used."
            )

        # If previous question was about strategy
    ...
    if (
        ...
        return (
            ...
            "comparison, you should also consider total money recovered "
            "and the value of transactions assigned to each strategy."
        )
    ...
    if (
        ...
        return (
            ...
            + "\n".join(lines)
            + "\n\n"
            "Different failure types can require different recovery "
            "actions, so they should be analyzed separately."
        )
